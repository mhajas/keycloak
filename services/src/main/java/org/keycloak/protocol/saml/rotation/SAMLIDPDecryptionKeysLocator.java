...
import org.keycloak.rotation.DecryptionKeyLocator;
 ...
 *
 */
public class SAMLIDPDecryptionKeysLocator implements DecryptionKeyLocator {

    private final KeycloakSession session;
    private final RealmModel realm;
    private final SAMLIdentityProviderConfig idpConfig;

    public SAMLIDPDecryptionKeysLocator(KeycloakSession session, RealmModel realm, SAMLIdentityProviderConfig idpConfig) {
    ...
    private Predicate<KeyWrapper> hasMatchingAlgorithm(String algorithm) {
        SAMLEncryptionAlgorithms usedAlgorithm = SAMLEncryptionAlgorithms
                .forXMLEncIdentifier(algorithm);
    ...
    public List<PrivateKey> getKeys(EncryptedData encryptedData) {
        ...
        if (encryptionAlgorithm != null && !encryptionAlgorithm.trim().isEmpty()) {
            keysStream = keysStream.filter(keyWrapper -> Objects.equals(keyWrapper.getAlgorithmOrDefault(), encryptionAlgorithm));
        }
