...
public abstract class AbstractKcSamlEncryptedElementsTest extends AbstractBrokerTest {
        ...
        public void setupKeys() {
            sigProviderId = ApiUtil.findActiveSigningKey(adminClient.realm(bc.consumerRealmName())).getProviderId();
            encProviderId = ApiUtil.findActiveEncryptingKey(adminClient.realm(bc.consumerRealmName())).getProviderId();
        ...
        public void testEncryptedElementIsReadable() throws ConfigurationException, ParsingException, ProcessingException {
            KeysMetadataRepresentation.KeyMetadataRepresentation activeEncryptingKey = ApiUtil.findActiveEncryptingKey(adminClient.realm(bc.consumerRealmName()));
        ...
        public void testSignatureKeyEncryptedElementIsNotReadableWithoutDeprecatedMode() throws ConfigurationException, ParsingException, ProcessingException {
            KeysMetadataRepresentation.KeyMetadataRepresentation activeSignatureKey = ApiUtil.findActiveSigningKey(adminClient.realm(bc.consumerRealmName()));
        ...
        public void testUseDifferentEncryptionAlgorithm() throws Exception {
            ...
            try (AutoCloseable ac = ApiUtil.generateNewRealmKey(realm, KeyUse.ENC, Algorithm.RSA1_5)) {
                KeysMetadataRepresentation.KeyMetadataRepresentation key = ApiUtil.findRealmKeys(realm, k -> k.getAlgorithm().equals(Algorithm.RSA1_5))
                        .findFirst()
