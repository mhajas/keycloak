...
[source,json,role="scrollable-code",id="keycloak-status"]
----
include::partials/keycloak-status.json[]
----
...
<script>
  document.getElementById('download-btn').addEventListener('click', function () {
    const jsonData = JSON.parse(document.getElementById("keycloak-status").textContent);
    const file = new Blob([JSON.stringify(jsonData, null, 2)], {type: 'application/json'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(file);
    a.download = 'keycloak-status.json';
    a.click();
