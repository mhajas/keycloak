...
public class ResourceAdapter implements Resource, CachedModel<Resource> {
    ...
    protected boolean isUpdated() {
        ...
        updated = cacheSession.getResourceStoreDelegate().findById(PermissionTicketStore.NULL_REALM, getResourceServer(), cached.getId());
        if (updated == null) throw new IllegalStateException("Not found in database");
        return true;
    }
    ...
    public ResourceServer getResourceServer() {
        return cacheSession.getResourceServerStoreDelegate().findById(PermissionTicketStore.NULL_REALM, cached.getResourceServerId());
    }
    ...
    public List<Scope> getScopes() {
        ...
        for (String scopeId : cached.getScopesIds(modelSupplier)) {
            scopes.add(cacheSession.getScopeStore().findById(PermissionTicketStore.NULL_REALM, getResourceServer(), scopeId));
        }
        ...
        for (Scope scope : updated.getScopes()) {
            if (!scopes.contains(scope)) {
                ...
                for (PermissionTicket permission : permissions) {
                    permissionStore.delete(PermissionTicketStore.NULL_REALM, permission.getId());
                }
            }
        }
    ...
    private Resource getResourceModel() {
        return cacheSession.getResourceStoreDelegate().findById(PermissionTicketStore.NULL_REALM, getResourceServer(), cached.getId());
    }
}
