...
public class StoreFactoryCacheSession implements CachedStoreFactoryProvider {
    ...
    private Set<String> getResourceTypes(Set<String> resources, String serverId) {
        ...
        ResourceServer resourceServer = getResourceServerStore().findById(PermissionTicketStore.NULL_REALM, serverId);
        return resources.stream().map(resourceId -> {
            Resource resource = getResourceStore().findById(PermissionTicketStore.NULL_REALM, resourceServer, resourceId);
    ...
    protected class ResourceServerCache implements ResourceServerStore {
        ...
        public void delete(ClientModel client) {
            ...
            if (id == null) return;
            ResourceServer server = findById(PermissionTicketStore.NULL_REALM, id);
        ...
        public ResourceServer findByClient(ClientModel client) {
            return findById(PermissionTicketStore.NULL_REALM, client.getId());
        }
    }
    ...
    protected class ScopeCache implements ScopeStore {
        ...
        public Scope findByName(ResourceServer resourceServer, String name) {
            ...
            if (query == null) {
                ...
                return findById(PermissionTicketStore.NULL_REALM, resourceServer, id);
            }
        }
    ...
    protected class ResourceCache implements ResourceStore {
        ...
        public Resource create(ResourceServer resourceServer, String id, String name, String owner) {
            ...
            Resource cached = findById(PermissionTicketStore.NULL_REALM, resourceServer, resource.getId());
            ...
            if (cached == null) {
                cached = findById(PermissionTicketStore.NULL_REALM, resourceServer, resource.getId());
            }
            return cached;
        }
        ...
        private <R extends Resource, Q extends ResourceQuery> List<R> cacheQuery(String cacheKey, Class<Q> queryType, Supplier<List<R>> resultSupplier, BiFunction<Long, List<R>, Q> querySupplier, ResourceServer resourceServer, Consumer<R> consumer, boolean cacheResult) {
            ...
            if (query == null) {
                ...
                if (consumer != null) {
                    resources.stream().map(resourceId -> (R) findById(PermissionTicketStore.NULL_REALM, resourceServer, resourceId)).forEach(consumer);
                } else {
                    model = resources.stream().map(resourceId -> (R) findById(PermissionTicketStore.NULL_REALM, resourceServer, resourceId)).collect(Collectors.toList());
                }
            }
    ...
    protected class PolicyCache implements PolicyStore {
        ...
        public Policy create(ResourceServer resourceServer, AbstractPolicyRepresentation representation) {
            ...
            Policy cached = findById(PermissionTicketStore.NULL_REALM, resourceServer, policy.getId());
            ...
            if (cached == null) {
                cached = findById(PermissionTicketStore.NULL_REALM, resourceServer, policy.getId());
            }
            return cached;
        }
            ...
            if (query == null) {
                ...
                if (consumer != null) {
                    for (String id : policies) {
                        consumer.accept((R) findById(PermissionTicketStore.NULL_REALM, resourceServer, id));
                    }
                } else {
                    model = policies.stream().map(resourceId -> (R) findById(PermissionTicketStore.NULL_REALM, resourceServer, resourceId))
    ...
    protected class PermissionTicketCache implements PermissionTicketStore {
        ...
        public void delete(RealmModel realm, String id) {
            ...
            getPermissionTicketStoreDelegate().delete(PermissionTicketStore.NULL_REALM, id);
        ...
        private <R, Q extends PermissionTicketQuery> List<R> cacheQuery(String cacheKey, Class<Q> queryType, Supplier<List<R>> resultSupplier, BiFunction<Long, List<R>, Q> querySupplier, ResourceServer resourceServer) {
            ...
            if (query == null) {
                ...
                return query.getPermissions().stream().map(resourceId -> (R) findById(PermissionTicketStore.NULL_REALM, resourceServer, resourceId)).collect(Collectors.toList());
            }
        }
    }
