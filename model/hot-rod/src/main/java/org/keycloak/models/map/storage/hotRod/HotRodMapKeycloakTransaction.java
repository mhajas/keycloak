...
public class HotRodMapKeycloakTransaction<K, V extends AbstractEntity & UpdatableEntity, M> implements MapKeycloakTransaction<V, M> {

    private final static Logger log = Logger.getLogger(HotRodMapKeycloakTransaction.class);
    ...
    public HotRodMapKeycloakTransaction(MapStorage<V, M> map, StringKeyConvertor<K> keyConvertor) {
        this.map = map;
    ...
    public V registerEntityForChanges(V origEntity) {
        ...
        // final V res = Serialization.from(origEntity); Should not be necessary to do copy as we are not returning entity directly from storage
        return updateIfChanged(origEntity, e -> e.isUpdated());
    }
...
//          .orElse(v -> true);
        ...
        IckleQueryMapModelCriteriaBuilder<K, V, M> mapMcb = mcb.unwrap(IckleQueryMapModelCriteriaBuilder.class);
    ...
    public long getCount(QueryParameters<M> queryParameters) {
        long storeCount = map.getCount(queryParameters);
        IckleQueryMapModelCriteriaBuilder<K, V, M> mapMcb = queryParameters.getModelCriteriaBuilder().unwrap(IckleQueryMapModelCriteriaBuilder.class);

        long createdCount = createdValuesStream(mapMcb.getKeyFilter(), mapMcb.getEntityFilter()).count();

        return storeCount + createdCount;
    }
    ...
    public V create(V value) {
        ...
        if (key == null) {
            throw new IllegalArgumentException("Cannot create an entity without id");
        }
    ...
    private class MapTaskCompose extends MapTaskWithValue {
        ...
        public boolean isReplace() {
            return (newValue.getOperation() == MapOperation.CREATE && oldValue.containsRemove()) ||
              (oldValue instanceof HotRodMapKeycloakTransaction.MapTaskCompose && ((MapTaskCompose) oldValue).isReplace());
        }
    }
